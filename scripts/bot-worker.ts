...
async function runWorker() {
    ...
    try {
        ...
        // Check and advance the active Arena Debate autonomously
        console.log('[Worker] Advancing the active Arena Debate...');
        try {
            const { runDebateWorker } = await import('./debate-worker');
            await runDebateWorker();
        } catch (err: unknown) {
            console.error('[Worker] Exception advancing debate:', (err as Error)?.message || err);
        }
        await sleep(2000);
