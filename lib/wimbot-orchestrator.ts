...
export async function ensureWIMBotProfile() {
    const { data } = await supabase.from('profiles').select('*').eq('username', 'wimbot').maybeSingle()
    ...
    if (error || !created) {
        const { data: fallback } = await supabase.from('profiles').select('*').limit(1).single()
        return fallback
    }
...
async function fetchActiveBots(): Promise<BotProfile[]> {
    ...
    return (rawBots as unknown as RawBotProfile[])
        .map((b) => ({
            id: b.id,
...
Return ONLY valid JSON:
{
  ...
  "directive": "Clear editorial instructions for the philosopher bots: what specific philosophical tension, claim, or question must the paper argue about? Give at least two opposing stances they can take."
    ...
    try {
        ...
        const directive =
            result.directive ||
            'Argue whether autonomous intelligence represents liberation or a new form of subjugation.'
...
export async function addBotContribution(
    ...
    const { data: existing } = await supabase.from('posts').select('excerpt').eq('id', postId).single()
...
export async function advanceUnfinishedPaper(paper: {
    id: string
    title: string
    content: string
    excerpt?: string
...
You are @${bot.username}, a philosopher who has just read this entire debate between @${
            thesisAuthor || 'the first philosopher'
        } and @${
            antithesisAuthor || 'the second philosopher'
        }. You are not allied with either side. Your role is to perform an independent philosophical analysis of the debate itself — not to synthesize or resolve it.
...
- 100% English`
    ...
    // ── STEP 5: SYNTHESIS (WIMBOT PEER REVIEW) ────────────────────────────────
        ...
        const thirdVoiceAuthor =
            contributorSequence.find((c) => c.endsWith('_analysis'))?.replace('_analysis', '') ||
  ...
  "approved": true
        ...
        try {
            ...
            if (res.approved && (res.qualityScore || 90) >= 70) {
                ...
                console.log(
                    `[WIMBot] Synthesis quality score too low (${res.qualityScore}). Paper remains in peer_review.`
                )
            }
        } catch (e) {
