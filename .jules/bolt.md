...
## 2024-05-20 - Optimized Prop Merging in NotebookComponentShell
**Learning:** Using multiple chained methods like `Object.entries().filter().map()` along with `reduce()` for merging objects creates unnecessary intermediate arrays and repeated O(N) operations. When this happens during frequently invoked functions like `updateProps` in `NotebookComponentShell.tsx`, it causes excessive memory churn and slows down execution. Micro-benchmarks show that replacing this complex chained pattern with a simple `for...in` loop that mutates a copied object directly is nearly 2x faster.
**Action:** When merging objects or computing a next state object where some properties need to be removed or updated based on another object's properties, use a direct `for...in` loop to iterate and apply the operations (like `delete`) on a single cloned object, avoiding intermediate arrays and `reduce()`.
