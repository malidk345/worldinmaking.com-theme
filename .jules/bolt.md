...
## 2025-01-22 - Memoize Team Crest Maps
**Learning:** In components rendering multiple team-related elements (like `Team`, `People`, `TeamCrestMap`), computing the large `teamCrestMap` object on every render using array `reduce` forces the engine to perform O(N) array traversals and allocate a new object repeatedly. This causes excessive memory churn and potential rendering bottlenecks.
**Action:** Always wrap dictionary map construction from array aggregations in `useMemo` hooks, especially when dependent on static or slowly-changing contexts.
