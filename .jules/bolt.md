...
## 2024-12-05 - Memoizing the Team Crest Map
**Learning:** In `src/hooks/useTeamCrestMap.ts`, `src/components/People/index.tsx`, and `src/components/Team/index.tsx`, computing the `teamCrestMap` object on every render using `.reduce()` forces the JavaScript engine to perform an O(N) array traversal and allocate a new object, which creates unnecessary memory churn and processing overhead.
**Action:** When computing derived maps for $O(1)$ lookups from an infrequently changing array of nodes, always wrap the `.reduce()` (or `Object.fromEntries()`) call in `useMemo`. This pattern should be consistently applied across all team/profile related components to reduce unnecessary rendering impact.
