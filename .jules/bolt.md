...
## 2024-11-20 - Flattening Arrays Efficiently
**Learning:** Using `reduce` combined with array spread syntax (`[...acc, ...cur]`) to flatten arrays inside React hooks causes an $O(N^2)$ time complexity bottleneck due to continuous array cloning on every iteration. In large or paginated data sets from SWR Infinite, this heavily blocks the main thread during render.
**Action:** Always use the $O(N)$ single-pass `.flatMap()` or `.flat()` methods instead of `reduce` with spread syntax for array flattening to prevent main thread blocking during React renders.
