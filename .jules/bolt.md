...
## 2024-12-05 - Memoizing team aggregations
**Learning:** In the `Team` and `People` components, computing arrays and mappings (e.g., `teamCrestMap`, `heightToHedgehogs`, `teamJobs`) using `.reduce` or `.filter` without `useMemo` forces the JavaScript engine to perform O(N) traversals and allocate new objects/arrays on every render, adding garbage collection pressure and main thread overhead on each React state update.
**Action:** Wrap object array aggregations (like `.reduce` or `.filter` results) in `useMemo` when they depend on statically queried `nodes` or props that don't change often.
