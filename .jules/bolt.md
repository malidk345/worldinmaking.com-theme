...
## 2024-05-18 - Remove O(N^2) object spread in array reduce
**Learning:** Using object spread (`...acc`) inside `.reduce()` loops causes O(N^2) time complexity and memory churn, heavily impacting performance for large iterations.
**Action:** Use single-pass O(N) methods like `Object.fromEntries(Object.entries().map(...))` instead of `.reduce()` for object accumulation.
