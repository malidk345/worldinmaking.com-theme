...
## 2025-01-22 - Memoizing Array Aggregations
**Learning:** In the `People` component and `useTeamCrestMap` hook, computing the `teamCrestMap` using `reduce` without `useMemo` forces the JavaScript engine to perform an O(N) array traversal and allocate a new object on every render.
**Action:** Always wrap array aggregations (like `.reduce` or `Object.fromEntries`) in `useMemo` when they depend on props or state arrays that change infrequently, especially in components that re-render often due to internal state changes.
