...
## 2024-12-05 - Lazily Initializing React State and Avoiding Array-As-Object Anti-pattern
**Learning:** In `src/components/Pricing/PricingCalculator/Tabbed.tsx`, initializing React state with `useState(analyticsSliders.reduce(...))` forces an O(N) array traversal and object construction to occur synchronously on *every single render*. Furthermore, initializing the `reduce` accumulator with an empty array `[]` and then assigning non-numeric string keys to it (`acc[type] = ...`) forces the JS engine (like V8) to de-optimize the array into 'dictionary mode', causing unnecessary memory churn and slowing down operations.
**Action:** When deriving initial component state from complex static or semi-static data, wrap the initialization in a lazy initializer function (`useState(() => ...)`) so it only runs once on mount. Additionally, avoid the 'array-as-object' anti-pattern by using a plain object `{}` as the accumulator for dictionary-like operations.
