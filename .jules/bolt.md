...
## 2024-12-05 - Memoizing Team Crest Map Computation
**Learning:** Computing `teamCrestMap` using `.reduce()` without `useMemo` in components like `Team` and `People` forces the JavaScript engine to perform an O(N) array traversal and allocate a new dictionary object on every render. This creates unnecessary overhead and memory churn.
**Action:** Wrap object construction from array aggregations in `useMemo` when they depend on infrequently changing props or state arrays, preventing redundant work on every render cycle.
