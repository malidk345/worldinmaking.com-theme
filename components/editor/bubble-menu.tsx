...
import {
  ...
  ChevronDown, X, Link, Baseline, Highlighter, RemoveFormatting,
  Check, CodeSquare, Superscript, Subscript, ImageIcon
} from 'lucide-react'
...
function Sep() {
  return <div className="w-px h-4 bg-black/10 dark:bg-white/12 shrink-0 mx-0.5" />
}

function Btn({
  onClick, isActive, title, children, className,
}: {
  ...
  return (
    <button
      ...
      title={title}
      className={cn(
        'flex items-center justify-center h-6 w-6 rounded-md text-sm transition-all duration-100 shrink-0',
        isActive
          ? 'bg-foreground text-background'
          : 'text-foreground/60 hover:text-foreground hover:bg-black/8 dark:hover:bg-white/12',
        className
      )}
    >
      {children}
...
function ColorSwatch({ color, isSelected, onClick, label }: { color: string | null; isSelected: boolean; onClick: () => void; label: string }) {
  return (
    ...
    >
      {color === null && <>
        <span className="absolute inset-0 flex items-center justify-center"><X size={10} className="text-muted-foreground" /></span>
        <span className="absolute inset-0 rounded-md border border-border/60" />
      </>}
    </button>
  )
}
    ...
    >
      ...
      >
        ...
        <div className="relative" ref={blockRef}>
          <button
            type="button"
            onMouseDown={e => { e.preventDefault(); closeAll(); setBlockOpen(v => !v) }}
            className="flex items-center gap-1 h-6 px-1.5 rounded-md text-foreground/70 hover:text-foreground hover:bg-black/8 dark:hover:bg-white/12 transition-all whitespace-nowrap"
          >
            <ActiveIcon size={13} />
            <span className="text-[11px] font-medium hidden sm:block">{activeBlock.label}</span>
                ...
                {BLOCK_TYPES.map(bt => {
                  ...
                  return (
                    <button key={bt.label} type="button"
                      onMouseDown={e => { e.preventDefault(); bt.command(editor); setBlockOpen(false) }}
                      className={cn('flex items-center gap-2 w-full px-2 py-1.5 rounded-xl text-left text-xs transition-all',
                        bt.isActive(editor) ? 'bg-foreground/8 text-foreground font-medium' : 'text-foreground/65 hover:text-foreground hover:bg-black/5 dark:hover:bg-white/8'
                      )}
                    >
                      <Icon size={13} className="shrink-0" />
                      <span>{bt.label}</span>
                      {bt.isActive(editor) && <Check size={11} className="ml-auto text-primary" />}
                    </button>
                  )
                })}
              </motion.div>
        ...
        <div className="relative" ref={colorRef}>
          <button type="button"
            onMouseDown={e => { e.preventDefault(); closeAll(); setColorOpen(v => !v) }}
            title="Text color"
            className={cn('flex flex-col items-center justify-center h-6 w-6 rounded-md gap-[2px] transition-all', colorOpen ? 'bg-black/8 dark:bg-white/12' : 'text-foreground/60 hover:text-foreground hover:bg-black/8 dark:hover:bg-white/12')}
          >
            <Baseline size={11} />
            <div className="h-[3px] w-3.5 rounded-full" style={{ backgroundColor: currentColor ?? 'currentColor' }} />
          </button>
          <AnimatePresence>
            {colorOpen && (
              <motion.div
              ...
              >
                ...
                <div className="grid grid-cols-5 gap-1.5">
                  {TEXT_COLORS.map(c => (
                    <ColorSwatch key={c.label} color={c.value} label={c.label} isSelected={currentColor === c.value}
        ...
        <div className="relative" ref={hlRef}>
          <button type="button"
            onMouseDown={e => { e.preventDefault(); closeAll(); setHlOpen(v => !v) }}
            title="Highlight"
            className={cn('flex items-center justify-center h-6 w-6 rounded-md transition-all', (hlOpen || editor.isActive('highlight')) ? 'bg-black/8 dark:bg-white/12' : 'text-foreground/60 hover:text-foreground hover:bg-black/8 dark:hover:bg-white/12')}
            style={currentHl ? { color: currentHl } : undefined}
          >
            <Highlighter size={13} />
          </button>
          <AnimatePresence>
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {HIGHLIGHT_COLORS.map(c => (
                    <ColorSwatch key={c.label} color={c.value} label={c.label} isSelected={currentHl === c.value || (!currentHl && c.value === null)}
                      onClick={() => { c.value ? editor.chain().focus().setHighlight({ color: c.value }).run() : editor.chain().focus().unsetHighlight().run(); setHlOpen(false) }} />
                  ))}
                </div>
              </motion.div>
            )}
        ...
        <div className="relative">
          <Btn
            onClick={() => { closeAll(); setLinkUrl(editor.getAttributes('link').href ?? ''); setLinkOpen(v => !v); setTimeout(() => linkInputRef.current?.focus(), 50) }}
          ...
          <AnimatePresence>
            {linkOpen && (
              <motion.div
                initial={{ opacity: 0, scale: 0.95, y: -4 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -4 }}
                transition={{ type: 'spring', stiffness: 520, damping: 34 }}
                className="absolute top-full mt-2 right-0 z-[60] rounded-2xl border border-white/30 dark:border-white/10 bg-white/88 dark:bg-zinc-900/92 shadow-xl backdrop-blur-2xl p-3 w-60"
              >
                ...
                <div className="flex gap-1.5 mt-2">
                  <button type="button" onMouseDown={e => { e.preventDefault(); applyLink() }}
                    className="flex-1 text-[11px] font-medium py-1.5 rounded-lg bg-foreground text-background hover:opacity-90 transition-opacity">Apply</button>
                  {editor.isActive('link') && (
                    <button type="button" onMouseDown={e => { e.preventDefault(); editor.chain().focus().unsetLink().run(); setLinkOpen(false) }}
                      className="text-[11px] font-medium px-2.5 py-1.5 rounded-lg bg-destructive/10 text-destructive hover:bg-destructive/20 transition-colors">Remove</button>
                  )}
                </div>
              </motion.div>
            )}
        ...
        {/* ── Image ── */}
        <Btn onClick={() => { closeAll(); onInsertImage() }} title="Insert image">
          <ImageIcon size={13} />
        </Btn>
