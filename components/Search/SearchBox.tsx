...
export const SearchBox: React.FC<SearchBoxProps> = ({ placeholder, location, filter, className }) => {
    ...
    return (
        <button onClick={handleSearchBoxClick} className="flex items-center relative m-0 w-full">
            ...
            <div
                className={`pl-10 py-3 text-base text-left text-primary/40 hover:text-primary/60 dark:text-primary-dark/40 dark:hover:text-primary-dark/60 bg-primary dark:bg-accent-dark dark:text-primary-dark rounded-md w-full md:w-[300px] mdlg:w-[400px] lg:w-[375px] xl:w-[500px] border border-input border-b-3 active:border-primary/40 ${className}`}
            >
                {placeholder || 'Search...'}
            </div>
        </button>
