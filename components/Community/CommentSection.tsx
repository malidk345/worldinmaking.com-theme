    ...
    return (
        <div className={`mt-12 flex flex-col w-full max-w-full min-w-0 transition-all duration-400 ease-[cubic-bezier(0.25,1,0.5,1)] ${className}`}>
            ...
            <h3
                ...
                className="text-[20px] font-semibold tracking-tight text-black/90 dark:text-white/90 mb-6 lowercase transition-colors duration-400 ease-[cubic-bezier(0.25,1,0.5,1)]"
            >
                {t('comments.title')}
            </h3>
            ...
            {loading && posts.length === 0 ? (
                ...
                <ul className="not-prose m-0 p-0 list-none mb-8 flex flex-col gap-6">
                    {adaptedPosts.map((post) => (
                        <li
                            key={post.id}
                            className="relative pl-4 md:pl-5 border-l-[2px] border-black/5 dark:border-white/5 transition-all duration-400 ease-[cubic-bezier(0.25,1,0.5,1)] hover:border-black/20 dark:hover:border-white/20 active:scale-[0.97] active:ease-[cubic-bezier(0.16,1,0.3,1)]"
                        >
                            <ForumQuestionCard
                                question={post}
                                isComment={true}
