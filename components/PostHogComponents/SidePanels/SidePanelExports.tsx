  ...
  const sampleExports: ExportAsset[] =
    exportsList.length > 0
      ...
      : [
          {
            id: "1",
            filename: "funnel-analysis-q3.csv",
            format: "CSV",
            createdAt: "5m ago",
            expiresAfter: "7 days",
            status: "ready",
          ...
          {
            id: "3",
            filename: "user-cohort-export.csv",
            format: "CSV",
            createdAt: "2h ago",
            status: "pending",
          },
      ...
      <div className="flex flex-col gap-3 py-2">
        ...
        <div className="flex items-center justify-between pb-2 border-b border-[var(--border-3000)]">
          <p className="text-xs text-[var(--muted-3000)] leading-relaxed">
            Exports are generated asynchronously and auto-deleted after a
            period.
          </p>
          <LemonButton
            icon={
          ...
          sampleExports.map((asset) => (
            ...
            >
              <div className="flex flex-col gap-1 min-w-0">
                <span className="text-xs font-semibold text-[var(--text-3000)] truncate">
                  {asset.filename}
                </span>
                <div className="flex items-center gap-1.5 flex-wrap">
                  <LemonTag
                    type={FORMAT_COLORS[asset.format] ?? "muted"}
                    size="small"
                  >
                    {asset.format}
                  </LemonTag>
                  <span className="text-[10px] text-[var(--muted-3000)] font-mono">
                    {asset.createdAt}
                  </span>
                  {asset.expiresAfter && (
                    <span className="text-[10px] text-[var(--muted-3000)]">
                      · expires in {asset.expiresAfter}
                    </span>
                  )}
                  {!asset.downloaded && asset.status === "ready" && (
                    <span className="text-[10px] text-[var(--warning-3000)]">
                      · not downloaded
                    </span>
                  )}
                </div>
              ...
              <div className="shrink-0">
                {asset.status === "pending" ? (
                  <LemonButton
                    ...
                    type="secondary"
                    disabled
                    icon={<IconRefresh className="size-3.5 animate-spin" />}
                  ...
                  <LemonButton
                    ...
                    disabled
                    icon={
                      <IconWarning className="size-3.5 text-[var(--danger-3000)]" />
                    }
