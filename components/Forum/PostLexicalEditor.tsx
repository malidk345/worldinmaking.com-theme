...
const theme = {
  ...
  list: {
    ...
    listitem:
      "text-sm md:text-base leading-relaxed text-zinc-800 dark:text-zinc-200 lowercase",
  },
  quote:
    "border-l-[3px] border-zinc-300 dark:border-zinc-700 pl-4 italic my-3 text-zinc-500 dark:text-zinc-400 lowercase",
  text: {
    ...
    <div className="flex flex-wrap items-center gap-1 p-1.5 border-b border-zinc-200/50 dark:border-zinc-800/50 bg-white/70 dark:bg-[#1C1C1E]/70 backdrop-blur-[20px] sticky top-0 z-10 select-none">
      ...
      <button
        ...
        className={buttonClass(
          activeBlock === "bullet" || activeBlock === "unordered-list",
        )}
      ...
      <button
        ...
        className={buttonClass(
          activeBlock === "number" || activeBlock === "ordered-list",
        )}
...
export default function PostLexicalEditor({
  ...
  return (
    <LexicalComposer initialConfig={initialConfig}>
      <div
        className={`relative flex flex-col w-full h-full min-h-0 bg-white dark:bg-[#1C1C1E] border border-black/5 dark:border-white/5 rounded-[24px] shadow-sm overflow-hidden group focus-within:border-black/10 dark:focus-within:border-white/10 transition-all duration-300 ${className}`}
      >
        {/* Formatting Toolbar */}
        <CustomToolbar />
        ...
        <div className="relative flex-1 overflow-y-auto no-scrollbar min-h-0">
          <RichTextPlugin
            contentEditable={
              <ContentEditable className="apple-body outline-none focus:outline-none p-5 sm:p-7 dark:text-zinc-200 h-full min-h-[300px] antialiased" />
            }
            placeholder={
