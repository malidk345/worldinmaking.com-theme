...
export default function ForumRichText({
    ...
    return (
        <div className={`relative bg-gray-50 dark:bg-gray-900 rounded-[24px] border border-black/5 dark:border-white/5 shadow-inner flex flex-col overflow-hidden w-full max-w-full min-w-0 ${expandHeight ? 'flex-1 h-full min-h-0' : ''} ${wrapperClassName}`} {...getRootProps()}>
            ...
            <div
                className={`not-prose flex items-center p-1.5 md:p-2 ${boxed ? `border-b ${borderClass}/50` : `border-b ${borderClass}/30`} bg-transparent overflow-hidden`}
            >
            ...
            {/* Editor Area */}
            <div className={`relative bg-transparent transition-colors duration-300 ease-in-out focus-within:bg-white dark:focus-within:bg-[#1C1C1E] ${expandHeight ? 'flex-grow flex flex-col min-h-0 h-full' : 'min-h-[120px]'}`}>
                ...
                {isDragActive && (
                    <div className="bg-white/80 dark:bg-[#1C1C1E]/80 supports-[backdrop-filter]:backdrop-blur-[40px] z-10 flex items-center justify-center absolute w-full h-full inset-0 p-4">
            ...
            {/* Bottom Bar */}
            <div className="flex justify-between items-center p-2 md:p-3 border-t border-black/5 dark:border-white/5 bg-transparent">
