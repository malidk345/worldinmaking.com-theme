...
const MOCK_PROFILES: Profile[] = [
  {
    ...
    username: "max",
    avatar_url:
      "https://res.cloudinary.com/dmukukwp6/image/upload/v1710153303/posthog.com/contents/images/authors/james.png",
  },
  {
    id: "1",
    username: "james",
  ...
  },
  {
    ...
    username: "tim",
    avatar_url:
      "https://res.cloudinary.com/dmukukwp6/image/upload/v1710153303/posthog.com/contents/images/authors/tim.png",
  },
];
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <p className="m-0 font-bold text-sm text-primary truncate lowercase">
              {profile.username}
            </p>
            {profile.id === "max" && (
              <IconFeatures className="w-3.5 h-3.5 text-primary opacity-50 shrink-0" />
            )}
          </div>
          <p className="m-0 text-[10px] font-medium opacity-50 truncate lowercase tracking-wide">
            {profile.id}
          </p>
        </div>
      </button>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center px-3 py-2 border-b border-black/5 dark:border-white/5 bg-black/5 dark:bg-white/5">
        <span className="text-[10px] font-bold lowercase opacity-50 tracking-wider">
          mentions
        </span>
        <button
          onClick={onClose}
...
export default function ForumRichText({
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.configure({
        HTMLAttributes: {
          class:
            "rounded-[12px] max-w-full h-auto my-4 border border-primary/20",
        },
      }),
      CharacterCount,
      Highlight.configure({
        HTMLAttributes: {
          style:
            "background-color: rgba(0, 0, 0, 0.08); color: black; border-radius: 4px; padding: 2px 4px;",
        },
      }),
      Underline,
    ],
  ...
  const handleProfileSelect = (profile: Profile) => {
    ...
    if (match) {
      ...
      const mentionText =
        profile.id === "max"
          ? `@max `
          : `@${profile.username.toLowerCase().replace(" ", "_")}/${profile.id} `;
  ...
  return (
    <div
      className={`relative bg-white/60 dark:bg-black/60 supports-[backdrop-filter]:backdrop-blur-[20px] rounded-[24px] border border-black/5 dark:border-white/5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] flex flex-col overflow-hidden w-full max-w-full min-w-0 ${expandHeight ? "flex-1 h-full min-h-0" : ""} ${wrapperClassName}`}
      {...getRootProps()}
    >
          ...
          {buttons.map((button) => {
            if (button.separator) {
              return (
                <div
                  key={button.name}
                  className="w-[1px] h-5 bg-black/10 dark:bg-white/10 mx-1 shrink-0"
                />
              );
            }

            const isActive = button.isActive(editor);
            const isDisabled = button.canExecute
              ? !button.canExecute(editor)
              : false;
      ...
      {/* Editor Area */}
      <div
        className={`relative bg-transparent transition-colors duration-300 ease-in-out focus-within:bg-white/80 dark:focus-within:bg-black/80 ${expandHeight ? "flex-grow flex flex-col min-h-0 h-full" : "min-h-[120px]"}`}
      >
        <div
          className={`relative h-full ${expandHeight ? "flex-grow flex flex-col min-h-0 h-full" : ""}`}
        >
          {mentions && (
            <AnimatePresence>
              {showMentionProfiles && (
          ...
          {label && !!editor.getHTML() && (
            <label className="text-[10px] opacity-40 block font-bold mb-1 px-4 pt-3 lowercase tracking-wide">
              {label}
            </label>
          )}
          <EditorContent
            editor={editor}
            className={
              expandHeight ? "flex-1 flex flex-col min-h-0 h-full" : ""
            }
          />
        </div>
        ...
        {isDragActive && (
          <div className="bg-white/80 dark:bg-black/80 supports-[backdrop-filter]:backdrop-blur-[20px] z-10 flex items-center justify-center absolute w-full h-full inset-0 p-4">
            <div className="border-2 border-dashed border-primary/20 rounded-[16px] w-full h-full flex items-center justify-center bg-black/5 dark:bg-white/5">
              <p className="m-0 font-bold lowercase text-primary/60">
                drop image here
              </p>
            </div>
          </div>
