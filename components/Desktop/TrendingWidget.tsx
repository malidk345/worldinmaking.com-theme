                ...
                <div className="flex items-center gap-3 text-[11px] opacity-60">
                    ...
                    <div className="flex items-center gap-2">
                        <IconRefresh
                            ...
                            tabIndex={0}
                            className={`w-3 h-3 opacity-60 cursor-pointer hover:opacity-100 mr-1 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20 dark:focus-visible:ring-white/20 ${loading ? 'animate-spin' : ''}`}
                            onClick={fetchTopPosts}
                            onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && fetchTopPosts()}
                        />
                        <IconChevronLeft
                            role="button"
                            aria-label="Previous page"
                            tabIndex={currentPage === 0 ? -1 : 0}
                            className={`w-3.5 h-3.5 cursor-pointer hover:opacity-100 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20 dark:focus-visible:ring-white/20 ${currentPage === 0 ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}
                            onClick={() => setCurrentPage(0)}
                            onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && currentPage !== 0) setCurrentPage(0) }}
                        />
                        <IconChevronRight
                            role="button"
                            aria-label="Next page"
                            tabIndex={currentPage >= totalPages - 1 ? -1 : 0}
                            className={`w-3.5 h-3.5 cursor-pointer hover:opacity-100 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black/20 dark:focus-visible:ring-white/20 ${currentPage >= totalPages - 1 ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}
                            onClick={() => setCurrentPage(prev => prev + 1)}
                            onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && currentPage < totalPages - 1) setCurrentPage(prev => prev + 1) }}
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="max-h-[220px] overflow-y-auto custom-scrollbar bg-transparent px-3 pb-3 flex flex-col gap-1">
                {currentPosts.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).map((post) => (
                    <div
                        ...
                        onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && activeTab !== 'updates') handleOpen(post) }}
