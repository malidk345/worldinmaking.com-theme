  ...
  const fetchTopPosts = async () => {
    ...
    try {
      ...
      const { data: bData, error: bError } = await supabase
        .from("posts")
        .select(
          "id, title, slug, view_count, author, author_id, created_at, profiles(username, avatar_url)",
        )
        .eq("published", true)
      ...
      const { data: cData, error: cError } = await supabase
        ...
        .select(
          "id, title, view_count, created_at, profiles(username, avatar_url)",
        )
        .is("post_slug", null)
        .order("view_count", { ascending: false })
        .limit(10);
      ...
      setBlogPosts(
        bData?.map((p) => {
          const profiles = p.profiles as unknown as {
            username?: string;
            avatar_url?: string;
          } | null;
          return {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-[var(--border-3000)]">
        ...
        <div className="flex items-center gap-3 text-[11px] opacity-65">
          <span className="lowercase text-[10px] font-bold font-mono">
            ...
            {Math.min((currentPage + 1) * itemsPerPage, currentPosts.length)} of{" "}
            {currentPosts.length}
          </span>
                      ...
                      <tr
                        ...
                        onMouseEnter={(e) => {
                          (
                            ...
                            "var(--color-bg-fill-button-tertiary-hover, rgba(0,0,0,0.04))";
                        }}
                        onMouseLeave={(e) => {
                          (
                            e.currentTarget as HTMLTableRowElement
                            ...
                            {post.avatar_url ? (
                              ...
                              <div
                                style={{
                                  ...
                                  borderRadius: "50%",
                                  background:
                                    "var(--color-bg-fill-button-tertiary-hover)",
                                  border: "1px solid var(--border-3000)",
                                  flexShrink: 0,
                                  display: "flex",
                                  alignItems: "center",
      ...
      <div className="px-4 py-2 border-t border-[var(--border-3000)] flex justify-between items-center text-[9px] opacity-50 lowercase font-bold font-mono">
        <div className="flex gap-3">
          {activeTab === "blog" ? (
            <span>
              1-{Math.min((currentPage + 1) * itemsPerPage, blogPosts.length)}{" "}
              of {totalPosts} posts
            </span>
          ) : (
            <span>
              1-{Math.min((currentPage + 1) * itemsPerPage, comPosts.length)} of{" "}
