...
test.describe('HumanTurnCard answer history wiring', () => {
    test('ChatMessage persists and renders ask_user answer / revise note', async () => {
        ...
        expect(index).toContain("action === 'answer' ? { answer: trimmed.slice(0, 2000) }")
    ...
    test('ask_user empty Enter / empty answer must not auto-continue as Yes', async () => {
        const { readFileSync } = await import('node:fs')
        const { resolve } = await import('node:path')
        ...
        expect(input).toContain("if (pendingHumanTurn && pendingHumanTurn.kind === 'ask_user')")
        expect(input).toContain('if (!trimmed')
        ...
        expect(index).toContain("void handleSendMessage(trimmed, [], { agentMode: chat.agentMode || 'ask' })")
