...
test.describe('worldinmaking seo', () => {
    ...
    test('home html has lowercase title, description, canonical, og, json-ld', async ({ request }) => {
        ...
        expect(html).toMatch(/name="description"[^>]*content="an AI markdown notebook and spatial desktop OS/i)
        expect(html).not.toContain('/posts?q={search_term_string}')
        ...
        expect(html).toMatch(/<h1>a desktop for writing<\/h1>/i)
