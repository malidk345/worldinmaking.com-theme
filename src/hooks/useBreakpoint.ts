    ...
    useEffect(() => {
        const update = () => {
            setBreakpoints((prev) => {
                ...
                // ⚡ Bolt Performance: Shallow-compare properties to bail out of rendering via Object.is equality if breakpoints haven't changed
                if (
                    prev.xs === next.xs &&
                    prev.sm === next.sm &&
                    prev.md === next.md &&
        ...
        update()
        // ⚡ Bolt Performance: Add passive: true to prevent UI thread blocking during high-frequency events
