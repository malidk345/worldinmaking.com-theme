...
function PostCard({ post, featured = false }: { post: any; featured?: boolean }) {
            ...
            <Link href={href} className="group block border border-primary rounded-lg overflow-hidden hover:bg-accent/20 transition-colors h-full">
                {imageUrl && (
                    <div className="aspect-video overflow-hidden bg-accent/30">
                        <CloudinaryImage src={imageUrl} width={800} imgClassName="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-500" alt={title} />
                    </div>
                )}
                <div className="p-5">
                    <span className="text-[10px] uppercase tracking-widest font-bold text-muted bg-accent/40 px-2 py-0.5 rounded mb-3 inline-block">Featured</span>
                    <h2 className="text-lg font-bold leading-snug mb-2 group-hover:underline">{title}</h2>
                    {attr.excerpt && <p className="text-sm text-secondary line-clamp-2 mb-4">{attr.excerpt}</p>}
                    <div className="flex items-center gap-2 text-xs text-muted">
                        {authors[0] && <AuthorAvatar profile={authors[0]?.attributes?.profile?.data?.attributes} className="size-5" />}
                        <span>{date}</span>
                    </div>
                </div>
            </Link>
    ...
    return (
        <Link href={href} className="group flex items-start gap-3 py-3 border-b border-primary last:border-b-0 hover:bg-accent/20 -mx-3 px-3 rounded transition-colors">
            {imageUrl && (
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5 text-xs text-muted">
                    {authors[0] && <AuthorAvatar profile={authors[0]?.attributes?.profile?.data?.attributes} className="size-4" />}
                    <span>{date}</span>
                </div>
            </div>
        </Link>
...
export default function LatestWriting() {
    const { posts, isLoading } = usePaginatedPosts({ params: { sort: ['date:desc'] }, pageSize: 7 })
    ...
    return (
        <section className="px-4 @xl:px-10 py-10 @xl:py-12 border-b border-primary">
            ...
            {isLoading ? (
                ...
                <div className="grid @xl:grid-cols-2 gap-8">
                    <div className="flex flex-col">{featured && <PostCard post={featured} featured />}</div>
