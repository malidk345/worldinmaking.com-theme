...
export async function loadCurrentWimUser(): Promise<User | null> {
    ...
    await claimThisDeviceIfNeeded(session.user.id, session.access_token)
...
export async function signInWithPassword(email: string, password: string): Promise<{ user: User | null; error?: string }> {
    ...
    const user = mapSupabaseToUser(data.user, profile)
    await claimThisDeviceIfNeeded(data.user.id, data.session?.access_token)
    return { user }
}
