            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 flex flex-col items-center" ref={threadRef}>
                <div className="w-full max-w-2xl flex flex-col gap-6 items-center">
                    ...
                    {messages.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-6 text-center space-y-3 w-full">
                            ...
                            {showBanner && (
                                <div className="w-full max-w-lg mt-2">
                                    <LemonBanner type="ai" onClose={() => setShowBanner(false)}>
                                        PostHog AI uses third-party LLM providers. Your data will not be used for
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 w-full pt-3">
                                {[
                                    {
                                        label: 'Analyze web traffic',
                                        icon: '📈',
                                        query: 'Show me $pageview telemetry trends for the last 7 days',
                                    },
                                    {
                                        label: 'Debug DOM click logs',
                                        icon: '🔍',
                                        query: 'Query $autocapture button clicks on main landing page',
                                    },
                                    {
                                        label: 'User identification',
                                        icon: '👤',
                                        query: 'How do I bind user traits with $identify in HogQL?',
                                    },
                                ].map((sug, i) => (
                                    <button
                                        key={i}
                    ...
                    {messages.map((msg) => (
                        <div
                            key={msg.id}
                            className={`w-full flex flex-col ${msg.type === 'human' ? 'items-end' : 'items-start'}`}
                        >
