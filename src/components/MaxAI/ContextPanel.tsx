...
const AVAILABLE_CONTEXTS: ContextItem[] = [
    ...
    },
    {
        ...
        category: 'event',
        description:
            'Automatically captures DOM element interactions like button clicks, link clicks, and form submissions.',
        samplePayload: {
            event: '$autocapture',
            distinct_id: 'usr_89234912',
            properties: {
...
export const ContextPanel: React.FC<ContextPanelProps> = ({ isOpen, onClose, onAttachContext }) => {
    ...
    return (
        <div
            id="context-panel"
            className="fixed inset-y-0 right-0 z-30 w-full sm:w-96 bg-light dark:bg-dark border-l border-accent-light dark:border-accent-dark shadow-xl flex flex-col transition-all duration-300 font-sans"
        >
                        ...
                        <div className="p-3 bg-accent-light/50 dark:bg-accent-dark/50 rounded-lg border border-accent-light dark:border-accent-dark">
                            ...
                            <ul className="space-y-1.5 font-code text-[11px]">
                                {Object.keys(selectedContext.samplePayload?.properties || {}).map((prop) => (
                                    <li
                                        key={prop}
                                        className="flex items-center justify-between py-0.5 border-b border-accent-light/30 dark:border-accent-dark/30 last:border-0"
                                    >
