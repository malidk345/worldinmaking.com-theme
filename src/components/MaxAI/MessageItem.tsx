...
export const MessageItem: React.FC<MessageItemProps> = ({ message, onRetry, onFeedback }) => {
        ...
        return parts.map((part, index) => {
            if (part.startsWith('```') && part.endsWith('```')) {
                ...
                return (
                    <div
                        key={index}
                        className="my-2.5 rounded-lg overflow-hidden border border-slate-700 bg-slate-900 text-slate-100 font-code text-xs"
                    >
            ...
            return (
                <div
                    key={index}
                    className="space-y-2 text-slate-800 dark:text-slate-200 text-sm leading-relaxed font-sans"
                >
                    {part.split('\n\n').map((paragraph, pIdx) => {
    ...
    return (
        <div
            id={`msg-${message.id}`}
            className="flex flex-col items-start my-4 max-w-2xl mr-auto space-y-2 w-full font-sans"
        >
            {message.thinkingSteps && message.thinkingSteps.length > 0 && (
                <ThinkingProcess steps={message.thinkingSteps} thinkingTimeMs={message.thinkingTimeMs} />
            )}
