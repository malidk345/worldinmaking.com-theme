...
export interface OfficialPostHogAIAppProps {
    onClose?: () => void
}
...
export function OfficialPostHogAIApp({ onClose }: OfficialPostHogAIAppProps): JSX.Element {
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[#f4f4f3] dark:bg-[#18191c] text-[#1d1f26] dark:text-[#f0f0f0] font-sans select-none relative overflow-hidden">
            {/* ── Top Header Toolbar ── */}
            <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-[#23252e] border-b border-[var(--lemon-border)] flex-shrink-0">
                <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-md bg-blue-600/10 text-blue-600 flex items-center justify-center font-bold text-xs font-mono">
                        🦔
                    </div>
                    <span className="font-bold text-sm font-rounded text-[var(--lemon-text-primary)]">PostHog AI</span>
                ...
                <div className="w-full max-w-2xl flex flex-col gap-6">
                    ...
                    {messages.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-8 text-center space-y-4">
                            <div className="w-14 h-14 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-lg text-2xl">
                                🦔
                            </div>
                            <div className="space-y-1">
                                ...
                                <p className="text-xs text-[var(--lemon-text-secondary)] italic">
                                    Build something people want with PostHog AI.
                                </p>
                            </div>
                            ...
                            {msg.type === 'ai' && (
                                <div className="flex flex-col gap-2 max-w-[95%] w-full">
                                    ...
                                    {msg.reasoning && (
                                        <div className="bg-[var(--lemon-bg-alt)] border border-[var(--lemon-border)] rounded-xl p-3 text-xs space-y-1 font-mono">
                                            ...
                                            {msg.reasoning.map((step: string, idx: number) => (
                                                <div
                                                    key={idx}
                                                    className="flex items-center gap-2 text-[11px] text-[var(--lemon-text-secondary)]"
                                                >
                                        ...
                                        {msg.content.split('```').map((chunk: string, i: number) => {
                                            if (i % 2 === 1) {
                                                return (
                                                    <div
                                                        key={i}
                                                        className="my-2 rounded-xl overflow-hidden border border-slate-700 bg-slate-900 text-slate-100 font-code text-xs"
                                                    >
                                    ...
                                    <div className="flex items-center gap-2 text-[var(--lemon-text-muted)] px-1 pt-1">
                                        <button
                                            className="hover:text-[var(--lemon-text-primary)] transition-colors p-1 rounded hover:bg-[var(--lemon-bg-muted)]"
                                            title="Helpful"
                                        >
            ...
            {/* ── Floating PostHog AI Composer Box at Bottom ── */}
                        ...
                        <div className="flex items-center gap-2 relative">
                            ...
                            {showContextPicker && (
                                <LemonCard className="absolute left-16 bottom-full mb-2 w-52 p-2 flex flex-col gap-1 z-50 text-xs shadow-xl">
                                    <div className="text-[10px] font-bold text-[var(--lemon-text-muted)] uppercase px-2 py-1">
                                    ...
                                    >
                                        <IconGraph style={{ width: 14, height: 14 }} /> $pageview event
                                    </button>
                                    <button
                                        className="text-left px-2 py-1.5 hover:bg-[var(--lemon-bg-muted)] rounded flex items-center gap-2 cursor-pointer"
                                        onClick={() => {
                                    ...
                                    >
                                        <IconDashboard style={{ width: 14, height: 14 }} /> $autocapture event
                                    </button>
                                    <button
                                        className="text-left px-2 py-1.5 hover:bg-[var(--lemon-bg-muted)] rounded flex items-center gap-2 cursor-pointer"
                                        onClick={() => {
                                    ...
                                    >
                                        <IconNotebook style={{ width: 14, height: 14 }} /> $identify person
                                    </button>
                                </LemonCard>
                            )}
                        </div>
