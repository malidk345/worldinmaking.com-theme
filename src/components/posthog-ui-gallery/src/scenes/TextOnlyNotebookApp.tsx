...
import { LemonButton, LemonTag, ProfilePicture, LemonCheckbox, LemonTextArea } from '../components/lemon-ui'
import { IconPlus, IconTrash, IconNotebook } from '../components/icons'
...
export function TextOnlyNotebookApp({
    ...
    const [blocks, setBlocks] = useState<NotebookBlock[]>([
        ...
        },
        {
            ...
            type: 'query',
            content:
                'SELECT step_name, count(distinct person_id) FROM events WHERE event = "checkout_step" GROUP BY step_name ORDER BY count DESC',
            queryOutput: [
            ...
            commentReplies: [
                {
                    id: 'cr-1',
                    author: 'James Green',
                    text: 'SeaweedFS payload limit might be hit during activity spikes.',
                    time: '2 hours ago',
                },
                {
                    id: 'cr-2',
                    author: 'Alex Smith',
                    text: 'Investigating Temporal worker execution logs right now.',
                    time: '1 hour ago',
                },
            ],
        },
            ...
            <div style={{ maxWidth: '900px', margin: '0 auto', width: '100%' }}>
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                        {onBack && (
                            <LemonButton size="small" type="tertiary" icon={<IconArrowLeft />} onClick={onBack}>
                                Back to Notebooks
                            </LemonButton>
                        )}
                                ...
                                >
                                            ...
                                            <LemonButton
                                                ...
                                                onClick={() =>
                                                    setActiveSlashBlockId(
                                                        activeSlashBlockId === block.id ? null : block.id
                                                    )
                                                }
                                            >
                                                + Add block
                                    ...
                                    {block.type === 'paragraph' && (
                                        <p
                                            ...
                                            suppressContentEditableWarning
                                            onBlur={(e) =>
                                                handleUpdateContent(block.id, e.currentTarget.textContent || '')
                                            }
                                            style={{
                                                outline: 'none',
                                                fontSize: '0.9375rem',
                                        ...
                                        >
                                            ...
                                            >
                                                <div style={{ fontWeight: 700, fontSize: '0.875rem' }}>
                                                    📊 Funnel Breakdown Node
                                                </div>
                                                <LemonTag type="highlight">Trends</LemonTag>
                                            </div>
                                            <LemonTextArea
                                                value={block.content}
                                                onChange={(val) => handleUpdateContent(block.id, val)}
                                                rows={2}
                                            />
                                            {block.queryOutput && (
                                                <div
                                                    ...
                                                    {block.queryOutput.map((cd) => (
                                                        ...
                                                        >
                                                            ...
                                                            >
                                                                <span>{cd.label}</span>
                                                                <strong>
                                                                    {cd.value.toLocaleString()} ({cd.percentage})
                                                                </strong>
                                                            </div>
                                                            <div
                                                                style={{
                                        ...
                                        >
                                            ...
                                            {block.commentReplies?.map((cr) => (
                                                <div
                                                    ...
                                                    style={{
                                                        paddingLeft: '0.75rem',
                                                        borderLeft: '2px solid var(--border-3000)',
                                                        fontSize: '0.8125rem',
                                                    }}
                                                >
                                    ...
                                    {activeSlashBlockId === block.id && (
                                        ...
                                        >
                                            ...
                                            <LemonButton
                                                size="small"
                                                type="tertiary"
                                                onClick={() => handleAddBlock(block.id, 'paragraph')}
                                            >
                                                📝 Paragraph
                                            </LemonButton>
                                            <LemonButton
                                                size="small"
                                                type="tertiary"
                                                onClick={() => handleAddBlock(block.id, 'h1')}
                                            >
                                                # Heading 1
                                            </LemonButton>
                                            <LemonButton
                                                size="small"
                                                type="tertiary"
                                                onClick={() => handleAddBlock(block.id, 'h2')}
                                            >
                                                ## Heading 2
                                            </LemonButton>
                                            <LemonButton
                                                size="small"
                                                type="tertiary"
                                                onClick={() => handleAddBlock(block.id, 'query')}
                                            >
                                                📊 Trends Query
                                            </LemonButton>
                                            <LemonButton
                                                size="small"
                                                type="tertiary"
                                                onClick={() => handleAddBlock(block.id, 'code')}
                                            >
                                                💻 HogQL SQL Code
                                            </LemonButton>
                                            <LemonButton
                                                size="small"
                                                type="tertiary"
                                                onClick={() => handleAddBlock(block.id, 'checklist')}
                                            >
                                                ☑ Checklist Item
                                            </LemonButton>
                                        </div>
