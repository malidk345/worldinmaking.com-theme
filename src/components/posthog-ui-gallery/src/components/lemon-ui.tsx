...
export function LemonDropdown({
    ...
    const childElement = children as
        | React.ReactElement<{ onClick?: (e: React.MouseEvent) => void; active?: boolean }>
        | undefined
    const triggerChild =
        childElement && React.isValidElement(childElement) ? (
...
export function LemonBadge({
    ...
    if (children) {
        return (
            <span
                className={clsx('LemonBadgeWrapper', className)}
    ...
    return (
        <span
            ...
            style={{
                ...
                backgroundColor:
                    status === 'danger'
                        ? 'var(--color-bg-fill-error-secondary, #fee2e2)'
                        : status === 'success'
                        ? 'var(--color-bg-fill-success-secondary, #dcfce7)'
                        : status === 'warning'
                        ? 'var(--color-bg-fill-warning-secondary, #fef3c7)'
    ...
    return (
        <span
            ...
            style={{
                ...
                backgroundColor:
                    type === 'highlight'
                        ? 'var(--primary-highlight, rgba(30,58,138,0.1))'
                        : type === 'completion' || type === 'success'
                        ? 'rgba(22,163,74,0.12)'
                        : type === 'warning'
                        ? 'rgba(217,119,6,0.12)'
                        : type === 'danger'
    ...
    return (
        <div
            ...
            style={{
                ...
                backgroundColor:
                    type === 'error'
                        ? 'var(--color-bg-fill-error-tertiary, #fef2f2)'
                        : type === 'success'
                        ? 'var(--color-bg-fill-success-tertiary, #f0fdf4)'
                        : type === 'warning'
                        ? 'var(--color-bg-fill-warning-tertiary, #fffbeb)'
                        : 'var(--color-bg-fill-info-tertiary, #eff6ff)',
                border: `1px solid ${
                    type === 'error'
                        ? 'var(--color-border-error, #fca5a5)'
                        : type === 'success'
                        ? 'var(--color-border-success, #86efac)'
                        : type === 'warning'
                        ? 'var(--color-border-warning, #fde047)'
...
export function LemonSelect<T extends string | number>({
    value,
    onChange,
    options,
    size = 'medium',
...
export function LemonCollapsePanel({
    header,
    children,
        ...
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
            <span
                style={{
                    ...
                    border: '2px solid var(--border-3000)',
...
export function LemonTreeItem({
    ...
    return (
        <div style={{ paddingLeft: '0.75rem' }}>
            ...
            {open && children && (
                <div style={{ borderLeft: '1px dashed var(--border-3000)', marginLeft: '0.5rem' }}>{children}</div>
            )}
        </div>
    )
}
...
export function LemonLabel({ children, info }: { children: React.ReactNode; info?: string }): JSX.Element {
    return (
        ...
        >
            {children}
            {info && (
                <IconInfo aria-label={info} style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)' }} />
            )}
        </label>
    )
}

export function LemonField({
    label,
    error,
    help,
    children,
        ...
        >
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <div>
                    ...
                    {description && (
                        <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)' }}>{description}</div>
                    )}
                </div>
            </div>
            {sideAction}
...
export function LemonMenu({
    ...
    items: { label: React.ReactNode; icon?: React.ReactNode; onClick?: () => void; status?: 'danger' | 'default' }[]
}): JSX.Element {
    return (
        <div
            style={{
        ...
        >
            <div
                style={{
                    ...
                    height: '100%',
                    backgroundColor:
                        status === 'success' ? '#16a34a' : status === 'danger' ? '#dc2626' : 'var(--primary-3000)',
...
export function LemonCheckbox({
    ...
    return (
        <label
            className={clsx('LemonCheckbox', disabled && 'LemonCheckbox--disabled', className)}
            style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '0.5rem',
...
export function LemonSwitch({
    ...
    return (
        <label
            className={clsx('LemonSwitch', disabled && 'LemonSwitch--disabled', className)}
            style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '0.625rem',
...
export function LemonDivider({
    ...
    return (
        <div
            className={clsx('LemonDivider', vertical && 'LemonDivider--vertical', className)}
            style={{
...
export function Link({
    ...
    return (
        <a
            ...
            style={{ color: 'var(--primary-3000)', textDecoration: 'none', fontWeight: 500 }}
        >
            {children}
        </a>
    )
...
export function UploadedLogo({ src, name = 'Logo' }: { src?: string; name?: string }): JSX.Element {
    return src ? (
        <img
            src={src}
            alt={name}
            style={{ width: '2rem', height: '2rem', borderRadius: 'var(--radius)', objectFit: 'contain' }}
        />
    ) : (
        <div
            style={{
