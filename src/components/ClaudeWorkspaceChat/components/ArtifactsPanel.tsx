...
import { X, Code2, Eye, Copy, Download, Check, Maximize2, Minimize2, ChevronDown } from 'lucide-react';
...
<body>
  ...
  <script type="text/babel">
    ${artifact.content}
    if (typeof LocalStorageDemo !== 'undefined') {
      ReactDOM.createRoot(document.getElementById('root')).render(<LocalStorageDemo />);
    } else if (typeof InteractiveCard !== 'undefined') {
      ReactDOM.createRoot(document.getElementById('root')).render(<InteractiveCard />);
    } else if (typeof App !== 'undefined') {
      ReactDOM.createRoot(document.getElementById('root')).render(<App />);
    } else {
      ReactDOM.createRoot(document.getElementById('root')).render(<p style={{color: '#6D6B67'}}>Bileşen yüklendi.</p>);
    }
  </script>
</body>
</html>`;
  ...
  const isInteractiveType = artifact.type === 'react' || artifact.type === 'html' || artifact.type === 'svg';
  ...
  return (
    <aside
      className={`absolute lg:relative inset-y-0 right-0 z-40 flex flex-col bg-white border-l border-primary transition-all duration-300 shadow-xl lg:shadow-none ${
        isFullscreen ? 'w-full lg:w-full' : 'w-full sm:w-[480px] lg:w-[580px] xl:w-[640px]'
      }`}
    >
      ...
      <div className={`flex items-center justify-between border-b border-primary px-3.5 py-2 shrink-0 select-none h-[48px] gap-2 font-claude-sans ${PANEL_BG}`}>
        ...
        <div className="flex items-center gap-3 flex-1 overflow-hidden min-w-0">
          ...
          <div className="flex items-center gap-1.5 min-w-0 truncate">
            ...
            <span className="text-secondary text-[11px] font-mono uppercase font-normal shrink-0">
              {getArtifactExtension(artifact).toUpperCase()}
            </span>
          </div>
        ...
        {/* Right Side Actions: Split Copy Button & Close */}
