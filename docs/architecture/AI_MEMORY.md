...
## 2026-09-14 - Jules\n**Learning:** When refactoring React components involving styling changes along with state logic changes, carefully extracting state to independent hooks (e.g. `useWindowMenu`) significantly simplifies the component tree and makes applying utilities like `cn` much more readable.\n**Action:** Always prefer using a `cn` utility to construct complex conditional strings over standard template literals, and enforce strict adherence to design memory artifacts like iOS 26.
