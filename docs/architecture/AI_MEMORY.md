...
- **Now:** Completed tools and a finished plan collapse in the same render as `isStreaming: false`, before the scroller measures. The post-settle frame restores from the sticky scroll position, not the cleared one-shot ref.
...
- **Changes:** (E1) `citation-export.ts`: BibTeX + RIS writers (types, escaping, citekeys, publisher/school, UTF-8 + RIS BOM) and a Blob download; Sources panel toolbar exports all verified sources of a reply; notebook Export panel gains "BibTeX (.bib)" / "RIS (.ris)" from footnotes + reference sections (`notebook-references.ts`, heuristic APA/MLA/Chicago parser + browser Crossref refresh for DOIs). (E2) `citation-styles.ts` (APA 7 default, MLA 9, Chicago author-date) on the shared `parsePersonName`; preference in `localStorage` (`citation-style-pref.ts`), selector in the Sources toolbar (`LemonSelect`) and a "Citations → Reference style" toggle in notebook Options (`ToggleGroup`); Copy / Add to notebook / whole-reply footnotes use it; `annotated_bibliography` gets a `style` argument and defaults to the browser preference sent as `citationStyle` (stored on `HostSnapshot.citationStyle`). (E3) `pdf-text.ts` wraps unpdf with byte/page/char/time caps and printed-page detection; `read_document` tries it first for PDFs (legacy scan kept as fallback, streamed 12 MB body cap instead of buffering the whole response), `find_quotes` reads all pages through `readRemotePdfPages` (one download; HTML pages still go through `read_document`). (E4) Trigger-first descriptions for `related_papers` / `find_quotes`, shorter routing line in `TOOL_PROTOCOL`, per-turn routing hint (`academic-routing.ts`, EN + TR) only on explicit asks; similar-paper topical-overlap filter + ranking, non-scholarly filter, OpenAlex topic fallback when `related_works` is empty, OpenAlex graph requests now select `topics`.
- **Files:** `src/lib/ai/{citation-styles,citation-export}.ts` (new), `src/lib/ai/{citation-format,contracts}.ts`, `src/lib/{citation-style-pref,notebook-references}.ts` (new), `src/lib/{notebook-citations,notebook-artifact-block}.ts`, `src/lib/bots/{pdf-text,academic-routing}.ts` (new), `src/lib/bots/{academic-bibliography,academic-citations,academic-graph,academic-quotes,academic-search}.ts`, `src/lib/bots/tools/{execute,host,read-document,spec}.ts`, `src/pages/api/chat.ts` (citationStyle + routing hint lines only), `src/components/ClaudeWorkspaceChat/{index.tsx,types.ts,components/SourcesPanel.tsx}`, `src/notebook-app/scenes/notebooks/notebookSidebarPanels.tsx`, tests alongside (+ `tools/__fixtures__/compressed-tr.pdf`, 22 KB generated fixture).
- **Prompt size:** tool specs +332 chars, `TOOL_PROTOCOL` +56 chars (≈ +100 tokens per turn, ~0.9 %); the routing hint (≈ 60–75 tokens) is added only on matching turns.
...
- **Changes:** (a) "Add to notebook" without an open editor opens the most recent real notebook (or creates "Research Notes") via the deduping `openNotebookWindow` and a queued send — no more `/notebooks` list window / "Could not reach the notebook" / duplicate windows. (b) Sticky selection is notebook-scoped, cleared on collapse and after one use; live selection wins. (c) `dispatchNotebookOsEventWithAck` + `requestId` echo; `SourcesPanel` / whole-reply Add show "Adding…" and "Added" only on a real ack; notebook nacks now reach the chat (the footnote handler called a non-existent `appActions.addToast` and threw before nacking). (d) Bibliography tool says "offered", not "added"; insert cap 8,000 → 20,000 (bibliography 40,000) with paragraph-boundary clipping and a `truncated` flag. (e) Duplicate DOI / URL / title → `duplicate_source` "Already in this notebook". (f) Whole-reply Add turns citation markers into footnotes with APA definitions; inserted footnotes renumber after the note's own and reuse an identical existing definition. (g) Serializer drops `\[`/`\]` escapes only when the re-parse is identical, so `> [!TIP]` callouts survive re-saves. (h) PDF export renders a "Footnotes" section. (i) Compact prior-turn citations (`priorCitations`) in the request, context and tool context; related_papers / find_quotes / bibliography can resolve an earlier [P#].
- **Files:** `src/components/ClaudeWorkspaceChat/{index.tsx,components/ChatMessage.tsx,components/SourcesPanel.tsx}`, `src/lib/{notebook-add-target,notebook-os-claim,notebook-insert-compose,notebook-citations}.ts` (new), `src/lib/{notebook-os-dispatch,notebook-chat-bind,notebook-artifact-block}.ts`, `src/lib/ai/{prior-citations (new),citation-markers}.ts`, `src/lib/bots/{orchestrate,academic-graph,academic-bibliography}.ts`, `src/lib/bots/tools/{execute,host,loop,pipeline}.ts`, `src/pages/api/chat.ts`, `src/notebook-app/App.tsx`, `src/notebook-app/lib/components/MarkdownNotebook/markdown.ts`, `src/notebook-app/scenes/notebooks/exportNotebookPdf.ts`; tests alongside.
- **Known leftovers:** the replace-selection handler in `notebook-app/App.tsx` still calls the non-existent `appActions?.addToast` (out of scope). The PDF writer prints callout markers (`[!TIP]`) literally (pre-existing).

### 2026-09-25 — Grok Bot / Cursor (fix: academic citations package B — [P@n] leak, duplicates, sub-agents, [n] false positives, APA, DOI-first, retractions)

- **Scope:** Audit bugs 2, 7, 8, 9, 10, 12 + DOI-shaped queries + retracted papers. Branch from `main` (`eeec21ed`), not stacked on #844; later merged `main` (`fc5ebb02`, incl. #844 answer recovery) — no code overlap beyond `pipeline.ts` (auto-merged). No Supabase, no quota logic, no fonts / wallpapers / animations.
- **Changes:** New `mergeToolCitations` (`academic-citations.ts`) replaces the gated `renumberToolCitations` call in `pipeline.runOneToolCall`: it runs for every academic tool result (also when `citations` is empty, so `[P@n]` → `[Pn]` always), maps each local id to an existing turn citation when it is the same source (academic: DOI / folded title / normalized URL — catches SEP "Moral Luck" found twice; web: URL) or to the next contiguous id, rewrites `[Pk]` / `[Source k - ` in one pass and returns only the new citations → no duplicate cards on cache hits / shared in-flight searches / repeated sources. `runTaskSubagent` passes `{ citations }` to nested `executeToolCall`, merges nested results the same way (sub-agent sees global ids) and `TASK_READ_TOOLS` now includes `related_papers` / `find_quotes` (renumbering was the only blocker; `annotated_bibliography` stays out — mutating). `citation-markers.ts`: accepts `[P@n]` (renders as the normal clickable marker), inline code with any backtick run, skips `[[n]](url)` link text and plain numeric `[n]` glued to an identifier / `)` (`arr[5]`, `f(x)[2]`) — flows into `verifyAnswerCitations`. APA (`citation-format.ts`): corporate / group authors kept verbatim (keyword + function-word heuristic, commas allowed), particles (van, von, de, da, der, al, bin, ibn, …) kept with the family name, hyphenated initials (`J.-P.`), suffixes (`Jr.`), Turkish İ only for Turkish names, "&" before the last author, up to 20 authors / 21+ → first 19 "…" last via new `authorCount`, no period after `?` / `!`; `formatApaBibliography` uses the same helpers. Source converters keep up to 20 authors (first 19 + last + `authorCount` beyond), ranking still reads the first 4. `chat.ts`: `withTimeout` → `runWithAbortBudget` (own AbortController linked to the turn; aborts the Crossref DOI lookups when the 4.5 s budget ends). `SourcesPanel`: row = `<li>` (background / hover) with the select `<button>` and the external `<a>` as siblings (same 52 px rows, same link position). DOI queries: `searchAcademicCorpus` extracts a DOI from `query` or `query_original`, resolves it via `lookupDoiViaCrossref` (now also returns the full record as `paper`, capped authors, `retracted`) + Unpaywall and lists it first; a bare DOI returns only that work (no keyword fan-out); unknown DOI → keyword fallback + "not found in Crossref" note. Retractions: Crossref `updated-by` (retraction / withdrawal / removal, incl. Retraction Watch) and `update-to` (notices), OpenAlex `is_retracted`, PubMed / Europe PMC publication types, "RETRACTED:" / "Notice of Retraction" / "Retraction notice" titles; `applyRetractionPolicy` drops them by default (always under `open_access_only`), keeps them last only when the query is about retractions; payload line says RETRACTED, cards get `AiCitation.retracted` → "Retracted" tag (existing tag style) in the card and list; `related_papers` applies the same policy. Search cache key gains `v: 2`.
- **Files:** `src/lib/bots/academic-{citations,search,graph}.ts`, `src/lib/bots/tools/pipeline.ts`, `src/lib/ai/{citation-markers,citation-format,contracts}.ts`, `src/pages/api/chat.ts`, `src/components/ClaudeWorkspaceChat/{components/SourcesPanel.tsx,types.ts}`; tests `src/lib/bots/tools/academic-citations.pkgB.test.ts` (new, 22), `src/lib/ai/citation-format.test.ts` (new, 30), `SourcesPanel.test.tsx` (+2), `academic-search.test.ts` (APA expectation).
- **Verify:** full vitest failing set identical to main (+54 tests, all green); `pnpm typecheck:shell` only the 5 pre-existing `App.tsx` errors; `pnpm lint:shell` 0 errors; Playwright tool-loop / agent-modes / ask-ai-harness failing set identical to main (11). Screenshots of the Retracted tag in `/workspace/wim-tasks/pkgB/`.

### 2026-09-25 — Grok Bot / Cursor (fix: empty answer after tools — audit bug 1)

- **Symptom:** 3 of 4 live academic chats (Gemini) and stored chats from 09-14 / 09-17 / 09-22 / 09-25 ended with "Completed requested tool actions." or its quality-gate rewrite ("…the abyss gazes back"). The post-tool answer round returned `ok:false`; `loop.ts` `fallbackSuccessFromPartial` shipped the generic string with no retry, dropped the provider error (never logged), and `applyQualityGate` rewrote it into persona prose and marked it `passed`.
- **Root cause (code):** any non-429 / non-auth / non-tool-protocol error on the post-tool round (5xx, network, or a `thought_signature` 400, which `isToolProtocolReject` deliberately excludes) was terminal once tools had run; the pipeline's empty-answer placeholder had the same fate. The provider's own error could not be recovered from logs (detail discarded, no worker log access, no provider key on the box) — the new logging captures it next time.
- **Changes:** `loop.ts` builds the existing fallback chain (family → key → model, same order) as a candidate list; on a failed/empty post-tool answer round it logs the failure and retries the answer once with tools disabled on the next candidate (same model when it is the last), from the gathered tool results flattened to plain text (`tools/answer-recovery.ts`, no function-call parts/signatures, 32k-char cap, `maxTokens` 8192). If that fails: short honest TR/EN message (`answerIncompleteMessage`, sources kept as cards) with `fallback:'answer_incomplete'`; action-only turns keep the old confirmation with `fallback:'actions_placeholder'`. `orchestrate.ts` skips the quality gate for any `fallback` output (no `qualityGate` flag, no critic call) and passes the raw question for language. `pipeline.ts` reports `roundFailure` / `answerMissing` / final `messages`. No recovery when the failed round already streamed public text or produced artifacts (old behavior). Auth/rate-limit and tool-protocol rejects keep the old full rerun on the next key/model. Quota logic, Supabase, fonts, wallpapers untouched.
- **Files:** `src/lib/bots/tools/{loop,pipeline,answer-recovery}.ts`, `src/lib/bots/orchestrate.ts`, `src/lib/bots/tools/answer-recovery.test.ts`, docs.
- **Follow-ups found (not fixed here):** Gemini/Anthropic adapters drop host context (`withThinkInstruction` / `withHostContext` edit `messages[0]`, but those adapters send the static `systemPrompt`), so Gemini's post-tool reflect and write nudges never reach the model; `pipeline.emitPublic` runs `stripLeakedToolMarkup` (which trims) per streamed chunk, gluing words at chunk edges until `done.fullText` replaces the bubble.

### 2026-09-25 — Grok Bot / Cursor (feature: academic search step 3 — related_papers, find_quotes, annotated_bibliography)

- **Scope:** Step 3 of the academic-search plan (on top of steps 1+2, merged as `a3dd4640`). Keyless-first; no embeddings/vectors, no Supabase, no new UI surface, quota logic untouched.
- **Changes:** New `related_papers` tool (`academic-graph.ts`): paper = `[P#]` from this turn, DOI, OpenAlex `W…`, S2 sha / `CorpusId:` / `PMID:` / `arXiv:` or an S2/OpenAlex/arXiv URL. Sources: Semantic Scholar citations / references / recommendations (serialized `s2Queue` ≥ 1.1 s, cool-down on 429), OpenAlex single-work `referenced_works` / `related_works` (keyless: ≤ 6 free single-work fetches; with key one `openalex:W1|W2` list call) + `cites:` list (needs key — keyless it 429s and is reported), OpenCitations index v2 citations/references (keyless DOIs), Crossref deposited `reference[]`, then one Crossref batch `filter=doi:…` for DOI-only hits. Merge with the Unicode-aware `mergeAcademicPapers`, seed dropped, RRF over list ranks + citations/OA/DOI (+ optional `focus` coverage); `direction:"all"` round-robins citing / reference / similar so the best-covered relation cannot crowd the others out; "similar"-only works must share a content stem with the seed. Per-list status line, `missingKeys` names only, all-failed → `ok:false, degraded` ("not evidence of no citations"). Cache namespace `related` 7 d (partial 30 min, never on total failure). New opt-in `find_quotes` (`academic-quotes.ts`): verbatim ≤ 320-char passages (≤ 3 default, 5 max, sentence / 2-sentence windows, verbatim-substring guard, garbage filter) from S2 snippet search (key only; otherwise `missing_key`), Europe PMC JATS full text (OA PMC; section labels), CORE `fullText` (`coreQueue`), and — only when none of those had full text — the OA PDF/page through the existing `read_document` reader ("text block N (approximate page)"). Cache namespace `quotes` 7 d. Payloads use `[P1]…` for new works and `[P@n]` for works already cited this turn; `renumberToolCitations` shifts local ids and rewrites `[P@n]` → `[Pn]`, so ids stay turn-global across search + related + quotes and no duplicate card is emitted (`matchTurnCitation`: DOI, else folded title). New `annotated_bibliography` (`academic-bibliography.ts`): entries `{paper, annotation}` resolved only against this turn's citations or a Crossref-confirmed DOI (anything else rejected), APA via `formatApaReference`, alphabetical markdown; `add_to_notebook:true` reuses the existing client-side `insert_notebook_block` action (no server storage). `executeToolCall` gains an optional context (`{ citations }`) passed by the pipeline. `related_papers` / `find_quotes` are plan-mode research tools and parallel/cached reads; `annotated_bibliography` is a mutating tool. Not available to nested `task` subagents (their citations are not renumbered).
- **Files:** `src/lib/bots/academic-{graph,quotes,bibliography}.ts` (new), `src/lib/bots/academic-{search,common,cache,citations}.ts`, `src/lib/bots/tools/{execute,spec,pipeline,labels}.ts`, `src/lib/bots/agent/modes.ts`, `src/components/ClaudeWorkspaceChat/components/{ChatMessage,ThinkingBangDots}.tsx` (tool-name wiring only), tests (`academic-graph.step3.test.ts`, `academic-quotes.step3.test.ts`, `tests/tool-loop.spec.ts` allowlist), docs.
- **Verify:** full vitest failing set identical to main (+28 new tests, all green); `pnpm typecheck:shell` only the 5 pre-existing `App.tsx` errors; `pnpm lint:shell` 0 errors; Playwright `tool-loop` + `harden-desk-paths` only the 3 pre-existing tool-loop failures. Live keyless check in the PR. Not merged; no Supabase changes.

### 2026-09-25 — Grok Bot / Cursor (feature: academic search step 2 — TR Dizin/CORE/DOAJ, SEP/IEP, RRF, shared cache, citation check, paper cards)

- **Scope:** Step 2 of the academic-search plan (stacked on step 1 / PR #841). Keyless sources only; no Supabase, no new visual language.
- **Changes:** New sources TR Dizin (`search.trdizin.gov.tr/api/defaultSearch/publication/`, `limit` is ignored server-side → always ~10 hits, PAPER only, all-caps titles recased with tr/en locale), CORE v3 (`/v3/search/works/` with trailing slash; serialized ≥ 2 s, no retry, 429 → per-source cool-down from `X-RateLimit-Retry-After`, capped 10 min), DOAJ articles (serialized ≥ 0.5 s, always OA). Routing: TR Dizin for Turkish query/`query_original`/`language:tr`/Turkish-studies topics; DOAJ + CORE for humanities, Turkish studies or `open_access_only`; SEP (`searcher.py` HTML) + IEP (`wp-json` search, ranked locally, one retry with the leading term) for philosophy only — title + URL + ≤ 300-char excerpt, `kind:'encyclopedia'`, never full text. New optional `query_original` arg: Turkish goes to TR Dizin + extra `crossref[tr]` / `openalex[tr]` (language:tr) jobs, English to the rest, then merged. Ranking = reciprocal rank fusion (k = 60) over per-source ranks + query-token coverage threshold (weak hits dropped and counted as `droppedWeak`), modest citation/OA bonuses. Shared Cloudflare `caches.default` cache (`academic-cache.ts`): search 3 d (partial coverage 30 min, never on total failure), Unpaywall/Crossref DOI 30 d (miss 1 d); no-op when `caches` is unavailable. Deterministic citation check in `api/chat.ts` after the final answer (≤ 4.5 s budget, 0 LLM tokens): cited `[P#]`/`[n]`, verbatim titles and DOIs are matched to the turn's results, unknown DOIs are looked up via Crossref `/works/{doi}`, and a second `citations` SSE event carries `verified` flags + `verification` summary; answer text untouched. `pipeline.ts` renumbers `[P#]` / `[Source N - ` per tool call by the running citation offset so markers equal chat.ts ids. Citation contract (`contracts.ts`, `types.ts`) gains optional `kind`, `authors`, `year`, `venue`, `citationCount`, `doi`, `oaUrl`, `pdfUrl`, `verified` (old jsonb rows still render). SourcesPanel renders academic items as cards (meta line, Open access / Encyclopedia entry / Cited by / Unverified tags, PDF · DOI · Open · Copy APA · Add to notebook with existing button styles; header "N papers · N encyclopedia entries" / "N sources"); ChatMessage turns `[P#]` / `[n]` / `[Source n]` into small numbered buttons that open the matching source (unknown ids → inert dashed marker). "Add to notebook" uses the existing `wimNotebookAddFootnote` event when there is a notebook selection, else the existing insert path.
- **Files:** `src/lib/bots/academic-{search,common,cache,sources-extra,encyclopedia,citations}.ts`, `src/lib/bots/tools/{execute,spec,pipeline}.ts`, `src/pages/api/chat.ts`, `src/lib/ai/{contracts,citation-markers,citation-format}.ts`, `src/components/ClaudeWorkspaceChat/{index.tsx,types.ts,components/SourcesPanel.tsx,components/ChatMessage.tsx}`, tests (`academic-search.step2.test.ts`, `academic-citations.test.ts`, `academic-encyclopedia.test.ts`, `pipeline.citation-numbering.test.ts`, `citation-markers.test.ts`, `SourcesPanel.test.tsx`, `ChatMessage.citations.test.tsx`), `vitest.config.ts` (automatic JSX runtime so .tsx component tests run), docs.
- **Verify:** vitest `src/lib/bots src/lib/ai src/components/ClaudeWorkspaceChat` (only the 6 pre-existing failures); full vitest failing-file set identical to main; `pnpm typecheck:shell` (only the 5 pre-existing `App.tsx` errors); `pnpm lint:shell` 0 errors; Playwright `harden-desk-paths` + `tool-loop` (only the 3 pre-existing tool-loop failures). Not merged; no Supabase changes.
